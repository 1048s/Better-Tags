...
class GitHubTagger(tk.Tk):
    ...
    def start_tag_creation_thread(self):
        ...
        self.update_status("하@는중", "orange")
    ...
    def create_tag(self):
        ...
        if not all([token, repo_url, tag_name, tag_message]):
            messagebox.showerror("오고@곡", "필@수 문구를 채@워주세요")
            self.update_status("오고@곡 필드 미@싱", "red")
            return

        match = re.search(r"github\.com/([^/]+/[^/]+)", repo_url)
        if not match:
            messagebox.showerror("오고@곡", "잘못된 깃@허브 URL 형식입니다")
            self.update_status("오고@곡 잘못된 URL", "red")
            return
        ...
        try:
            ...
            if not commit_sha:
                self.status_var.set(f"'{repo.default_branch}'에서 최@신 커@밋 가@져오는 중...")
                default_branch = repo.get_branch(repo.default_branch)
                commit_sha = default_branch.commit.sha
                self.commit_sha_var.set(commit_sha)

            self.status_var.set(f"커@밋 {commit_sha[:7]}에 태@그 '{tag_name}' 생@성 중...")
            ...
            messagebox.showinfo("성@공", f"태@그 '{tag_name}' 성@공적으로 '{repo_path}'에 생@성되었습니다!")
            self.update_status(f"성@공! 태@그 '{tag_name}' 생@성됨.", "green")

        except GithubException as e:
            error_message = f"깃@허브 API 오고@곡 ({e.status}): {e.data.get('message', '알 수 없는 오고@곡')}"
            messagebox.showerror("깃@허브 오고@곡", error_message)
            self.update_status(f"실@패: {e.data.get('message', '')}", "red")
        except Exception as e:
            messagebox.showerror("오고@곡", f"예@상치 못한 오고@곡가 발@생했습니다: {e}")
            self.update_status(f"실@패: {e}", "red")
